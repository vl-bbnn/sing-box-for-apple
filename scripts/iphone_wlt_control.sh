...
run() {
    ...
    case "$action" in
        ...
        explicit_saved_WLT_outbound_reachability) ;;
...
if "go_memory_limit_mib" in parameter_keys:
    memory_limit = parameters["go_memory_limit_mib"]
    if (isinstance(memory_limit, bool) or not isinstance(memory_limit, int)
            or not 24 <= memory_limit <= 45):
        raise SystemExit("candidate Go memory limit must be 24..45 MiB")
...
if result.get("schema") not in {1, 2, 3, 4, 5, 6, 7, 8}:
