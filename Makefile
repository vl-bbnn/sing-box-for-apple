...
archive_ios:
	...
	mkdir -p build
	set -o pipefail; xcodebuild archive -scheme SFI -configuration Release -destination 'generic/platform=iOS' -archivePath build/SFI.xcarchive -allowProvisioningUpdates $(XCODE_AUTH_FLAGS) 2>&1 | tee build/archive_ios.log | xcbeautify | grep -A 10 -e "Archive Succeeded" -e "ARCHIVE FAILED" -e "❌" || { status=$$?; echo "---- raw xcodebuild tail (archive_ios) ----"; tail -n 200 build/archive_ios.log; exit $$status; }
...
archive_macos_standalone_apple: $(OVERLAY_HELPER_PLIST)
	...
	xcodebuild archive -scheme SFM.System -configuration Release -archivePath build/SFM.System-arm64.xcarchive ARCHS=arm64 -allowProvisioningUpdates $(XCODE_AUTH_FLAGS) | xcbeautify | grep -A 10 -e "Archive Succeeded" -e "ARCHIVE FAILED" -e "❌"

archive_macos_standalone_intel: $(OVERLAY_HELPER_PLIST)
	rm -rf build/SFM.System-x86_64.xcarchive
	xcodebuild archive -scheme SFM.System -configuration Release -archivePath build/SFM.System-x86_64.xcarchive ARCHS=x86_64 -allowProvisioningUpdates $(XCODE_AUTH_FLAGS) | xcbeautify | grep -A 10 -e "Archive Succeeded" -e "ARCHIVE FAILED" -e "❌"

archive_macos_standalone_universal: $(OVERLAY_HELPER_PLIST)
	rm -rf build/SFM.System-universal.xcarchive
	xcodebuild archive -scheme SFM.System -configuration Release -archivePath build/SFM.System-universal.xcarchive -allowProvisioningUpdates $(XCODE_AUTH_FLAGS) | xcbeautify | grep -A 10 -e "Archive Succeeded" -e "ARCHIVE FAILED" -e "❌"
