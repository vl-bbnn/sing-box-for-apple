...
private final class WLTServiceLifecycleCoordinator: @unchecked Sendable {
  ...
  func requestClose() -> CloseDecision {
    lock.lock()
    defer { lock.unlock() }
    stopIntent = true
    stopCallbackComplete = false
    if let closeResult {
      return .completed(closeResult)
    }
    if closeInProgress || transitionLifecycle != nil {
      return .wait
    }
    closeInProgress = true
    return .perform(activeLifecycle)
  }
...
open class ExtensionProvider: NEPacketTunnelProvider {
  ...
  func stopService(
    ...
    journalCloseReason: String = "service_stop"
      ...
      func stopStage(_ stage: String) {
        PacketTunnelDiagnostics.appendStopStage(stage, operationID: stopOperationID)
      }
      stopStage("provider_close_enter")
      let closeLifecycle: UInt64
      switch serviceLifecycle.requestClose() {
      case .completed(let succeeded):
        stopStage(succeeded ? "provider_close_already_ok" : "provider_close_already_error")
        return succeeded ? .succeeded : .failed("close service failed")
      case .wait:
        stopStage("provider_close_busy")
        return .inProgress
  ...
  override open func stopTunnel(with reason: NEProviderStopReason) async {
    ...
    stopDiagnosticsHeartbeat()
      ...
      while serviceCloseResult == .inProgress {
