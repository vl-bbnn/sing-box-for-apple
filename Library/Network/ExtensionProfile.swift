...
private let logger = Logger(category: "ExtensionProfile")
  ...
  enum WLTStopTransaction {
    ...
    static func perform(prepare: () async throws -> Void, closeService: () async throws -> Void, stopTunnel: () -> Void, record: (WLTStopStage) -> Bool) async throws {
      var body: Result<Void, Error>; var receiptsComplete = true
      var receiptFailures: [String] = []
      func receipt(_ stage: WLTStopStage) { if !record(stage) { receiptsComplete = false; receiptFailures.append(stage.rawValue) } }
      receipt(.appPrepareEnter)
      do {
        try await prepare()
        receipt(.appPrepareOK)
        receipt(.appRPCEnter)
        do { try await closeService(); receipt(.appRPCOK); body = .success(()) }
    ...
    func prepareStop() async throws {
      ...
      try await WLTStopTransaction.perform(
        ...
        closeService: {
          ...
          _ = try await WLTStopClient.close(operationID: stopOperationID, timeout: 20) { message, budget in
            try await sendDiagnosticResponse(message, session: session, timeoutMillis: budget)
          }
        },
        stopTunnel: { manager.connection.stopVPNTunnel() },
        record: { PacketTunnelDiagnostics.appendStopStage($0.rawValue, operationID: stopOperationID) }
      )
    #else
