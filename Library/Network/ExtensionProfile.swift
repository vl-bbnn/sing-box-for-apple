...
private let logger = Logger(category: "ExtensionProfile")
  ...
  enum WLTStopTransaction {
    static func perform(
      ...
      defer {
        record("app_os_stop_enter")
        stopTunnel()
        record("app_os_stop_return")
      }
      record("app_prepare_enter")
      do {
        try await prepare()
      } catch {
        record("app_prepare_error")
        throw error
      }
      record("app_rpc_enter")
      do {
        try await closeService()
        record("app_rpc_return_ok")
      } catch {
        record("app_rpc_return_error")
        throw error
      }
    }
  }
