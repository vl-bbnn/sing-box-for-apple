...
enum WLTStopStage: String, CaseIterable {
  ...
  case appOwnerBound = "app_owner_bound"
...
final class WLTStopOrchestrator: @unchecked Sendable {
  struct Dependencies {
    ...
    var record: (WLTStopStage, String, UInt64) -> Bool
  }
  ...
  func request(operationID proposed: String, reason: String) -> WLTStopReply {
    ...
    DispatchQueue.global(qos: .utility).async { [self] in
      ...
      condition.unlock()
      let result = withExtendedLifetime(retainedOwner) { execute(operationID: proposed, lifecycle: token, reason: reason) }
      condition.lock()
  ...
  private func execute(operationID: String, lifecycle: UInt64, reason: String) -> WLTStopOutcome {
    var result = WLTStopOutcome()
    func receipt(_ stage: WLTStopStage) {
      if !dependencies.record(stage, operationID, lifecycle) { result.evidenceFailures.append("receipt:\(stage.rawValue)") }
    }
