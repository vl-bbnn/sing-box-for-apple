...
public class CommandClient: ObservableObject {
  ...
  public init(_ connectionTypes: [ConnectionType], logMaxLines: Int? = nil) {
  ...
  private func flushPendingLogs() {
    ...
    if let logMaxLines, logList.count > logMaxLines {
  ...
  private func filterConnections(_ message: LibboxConnections) -> (
    connections: [LibboxConnection], hasAny: Bool
  ) {
  ...
  private func initializeConnectionFilterState() async {
    let newFilter: ConnectionStateFilter =
      await .init(rawValue: SharedPreferences.connectionStateFilter.get()) ?? .active
    let newSort: ConnectionSort =
      await .init(rawValue: SharedPreferences.connectionSort.get()) ?? .byDate
    await MainActor.run {
  ...
  private class clientHandler: NSObject, LibboxCommandClientHandlerProtocol {
    ...
    func clearLogs() {
      // The core can emit clear events during setup/startOrReloadService.
      // Keep the UI buffer intact so early startup diagnostics are not lost;
      // manual clearing still goes through CommandClient.clearLogs().
      logger.debug("ignored remote clearLogs request")
    }
      ...
      DispatchQueue.main.async { [self] in
        ...
        if commandClient.logBatchTimer == nil {
          ...
          DispatchQueue.main.asyncAfter(
            deadline: .now() + commandClient.logBatchInterval, execute: workItem)
        }
      }
    }
